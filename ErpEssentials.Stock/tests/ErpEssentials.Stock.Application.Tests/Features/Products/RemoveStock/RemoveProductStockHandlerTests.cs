...
public class RemoveProductStockHandlerTests
{
    ...
    private readonly RemoveProductStockHandler _handler;

    public RemoveProductStockHandlerTests()
    {
        _mockProductRepository = new Mock<IProductRepository>();
        _mockUnitOfWork = new Mock<IUnitOfWork>();

        _handler = new RemoveProductStockHandler(
    ...
    {
        // Arrange
        var command = new RemoveProductStockCommand(Guid.Empty, 100);
    ...
    public async Task Handle_Should_Create_Lot_WhenDataIsValid()
    {
        ...
        var command = new RemoveProductStockCommand(
            product.Id,
            50
        );
